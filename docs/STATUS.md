...
| M2 — Deterministic legacy migration      | **In progress** | Real PostgreSQL/MinIO apply/replay plus exact content/media, Hero/About, ordered sections, private age, isolated rollback proof, and the reviewed accessible skill colours with a shared theme-contrast checker                                               | Apply `2026-08-15.skill-colors.1` against the real database and record the run                                                          | [M2](status/M2.md) |
...
1. ~~Resolve M2's three colour decisions~~ (done 2026-08-15) and apply the reviewed normalization version against the real database without changing the frozen source.

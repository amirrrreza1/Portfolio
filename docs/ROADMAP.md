...
Active milestone: **M6**. M2 is complete: its real PostgreSQL/MinIO apply/replay, public content/media, exact Hero/About, private age, ordered/disabled sections, and three reviewed accessible skill-colour replacements are all evidenced. M3 is complete: ADR-015's PostgreSQL article authority is proven against a real server — migration from zero, bilingual Markdown with database-recomputed digests, immutable revisions, rollback that leaves nothing behind, and idempotent publication that needs no network. M6's API security boundary is now demonstrated — two-factor login with cryptographically verified passkeys, enrolment, rotating opaque sessions, CSRF and origin enforcement, recovery with a session sweep, throttling, deny-by-default authorization, and redacted audit events, all against a running API. It remains open for the authenticated admin shell and the recovery drill. M4 is complete: the whole public path was exercised as one system on 2026-08-26 — publication, update, and withdrawal reaching the rendered route, a leakage matrix, one-hop legacy redirects, the outage and rollback matrix, and SMTP contact delivery. One item carries forward without holding the gate: nothing is currently served from a shared cache, so the signed invalidation pipeline purges nothing and every request fans out to the API. See §10 item 14. M5 is complete: its database-backed appearance allowlist, full THEMING §9 browser matrix, and owner-approved light-theme and screen-reader review are evidenced. M0's repository-owned work is complete; its gate is held open by one owner action, see §6.

...
Active milestones: **M4 and M6**. M2 is complete: its real PostgreSQL/MinIO apply/replay, public content/media, exact Hero/About, private age, ordered/disabled sections, and three reviewed accessible skill-colour replacements are all evidenced. M3 is complete: ADR-015's PostgreSQL article authority is proven against a real server — migration from zero, bilingual Markdown with database-recomputed digests, immutable revisions, rollback that leaves nothing behind, and idempotent publication that needs no network. M6 remains open until its API security boundary is demonstrated. M4 has five live-proven portfolio read boundaries and now also has an automated-verification-complete public article boundary: strict published-only APIs, bilingual routes, no-fallback locale behavior, current-render provenance, 15-minute bounded stale reads, article-route `503`, and published-only metadata. M5 is complete: its database-backed appearance allowlist, full THEMING §9 browser matrix, and owner-approved light-theme and screen-reader review are evidenced. M0's repository-owned work is complete; its gate is held open by one owner action, see §6.
...
M4 and M5 were deliberately pulled forward without waiting for M2/M3 to close because the root layout, locale shell, and colour usage are touched by every later surface. M5 and M3 are now complete. M3 closed on 2026-08-26: ADR-015's article authority is proven against a real PostgreSQL server, and a bilingual English/Persian article is published in the database. M4 adds an automated-verification-complete article read/route/SEO slice to its five live-proven portfolio boundaries, root negotiation, public catalogs, and server-side GitHub statistics; what it still needs is that live article rendered through the public route with publication invalidation proven end to end.
...
| Blog and admin               | Public bilingual blog list/detail routes now exist over strict API DTOs; authenticated authoring/publishing/admin routes do not. `packages/auth-core` supplies password/session/CSRF/recovery/WebAuthn challenge primitives.                                                                                                                                         | Real PostgreSQL-native article content now exists. End-to-end invalidation through the route, full passkey verification, authorization, and the admin shell remain.                                                |
...
M4 was opened before its M3 dependency exited. That was a real ordering exception, not a re-plan: the shell work was pulled forward because it is cheaper before more UI exists, and the parts of M4 that genuinely needed a proven PostgreSQL-native article foundation — real article reads and end-to-end invalidation — are exactly the parts still outstanding. M3 exited on 2026-08-26, so that exception is now discharged and those parts are unblocked. Two milestones remain open besides M0; no new milestone should open until the proof backlog contracts.
...
12. **M4 remaining public reads:** projects including detail/optional image mediation, site shell/sections, appearance, homepage collections, certificates, active resume, exact Hero/About, root negotiation/catalogs, and GitHub statistics have contract/client/render/rollback proof. The article contract, API, client, and routes are implemented; what remains is rendering M3's live bilingual article through them and proving publication invalidation end to end.

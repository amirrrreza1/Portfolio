...
Active milestone: **M9**, now in progress. M1–M8 are complete. M9's repository-owned operations foundation is implemented; deployment, restore-drill, independent-review, production-rollout, and rollback-window evidence remain open in [`status/M9.md`](status/M9.md). M0 remains blocked on its provider action and evidence reconciliation, but blocks nothing downstream. The caching, v1 editor-permission, and contact/audit retention decisions remain separate follow-ups.

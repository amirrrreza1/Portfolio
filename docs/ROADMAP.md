...
Active milestone: **M9**, now in progress. M1–M8 are complete. M9's repository-owned operations foundation, production-like local topology, and encrypted isolated local restore drill are implemented and evidenced; the production environment, independent review, production-sized restore, rollout, and rollback-window evidence remain open in [`status/M9.md`](status/M9.md). M0 remains blocked on its provider action and evidence reconciliation, but blocks nothing downstream. The caching, v1 editor-permission, and contact/audit retention decisions remain separate follow-ups.
...
| Appearance                   | The root layout resolves `portfolio_prefs` against a strict locale-scoped database/legacy owner allowlist and emits the result in the first byte; the dialog consumes that allowlist, and live database/rollback plus localized HTTP `503` outage proof exists                                                                                                              | Complete. The §3 token vocabulary, AA contrast matrix, true per-script WOFF2 subsets, route-scoped preload, self-only style CSP, and full §9 browser matrix are evidenced             |
| Operations and quality       | CI runs frozen install, format, lint, typecheck, tests, builds, migration/OpenAPI/license/audit gates, full-history secret scanning, hardened image builds, Trivy scans, and retained SPDX SBOM generation. The M9 maintenance, backup/restore, smoke, and runbook artifacts are implemented; the production-like topology and encrypted isolated local restore are proven. | Complete the independent review and production-sized restore, then retain production rollout/rollback-window evidence.                                                                |
...
The root quality commands prepare generated workspace-package artifacts themselves. The 2026-09-05 M9 release run has 828 passing tests across 81 files in eight tested apps/packages; the current post-fix verification has 833 assertions after adding contact-abuse and bounded trusted-proxy regressions. Every workspace typecheck, declared API/full web lint, library emit, API build, Next.js production build, and the 45-test public Chromium suite pass. Dedicated pnpm deployment lockfiles and injected workspace packages keep clean-checkout container builds reproducible.
...
| M9 — Operations, release, and cleanup     | In progress | L             | Operational contact/media controls, reproducible deployment, restore drill, launch, and rollback-window cleanup      | M5, M8             |
...
| ---------------------------------------------------------------------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The existing web and API builds pass from a clean checkout                         | **Met**                        | The strictly stronger [M9 local release proof](status/evidence/M9-local-release-proof.md) records the pinned clean-checkout format, lint, typecheck, 828-assertion test (833 in the current post-fix verification), production-build, browser, policy, container, and restore matrix on Node 24.13 / pnpm 11.9                                                    |
...
The original route screenshots, omitted during the first M0 pass, were recovered
from the frozen historical web tree on 2026-09-06 and are retained in the
[M0 legacy visual baseline](status/evidence/M0-legacy-visual-baseline.md).
...
screen-reader experience, closing M5. M9 later closed both non-gating follow-ups:
Vazir Code is re-encoded into disjoint Latin and Arabic/Persian WOFF2 files,
and Shiki's token palette is emitted as static classes so `style-src` is
self-only.
...
14. ~~**Caching ADR and implementation:** put validated public DTOs in Next 16's explicit shared cache without caching the visitor-specific shell, then prove targeted invalidation.~~ Done 2026-09-06 — ADR-016 enables Cache Components and a tagged `use cache` data boundary while ADR-009's shell remains request-bound. A production-build counting proxy measured zero upstream calls on an immediate repeat of `/en/blog`; expiring `public:articles:en` then refreshed exactly article list, taxonomy, and feed index without refetching site or appearance. Evidence: [`status/evidence/M9-shared-cache-proof.md`](status/evidence/M9-shared-cache-proof.md).
...
16. **Current milestone:** M9, operations, release, and cleanup. Its repository-owned implementation, local release/restore proof, and item 14's shared-cache boundary are complete. The overdue owner decisions (v1 editor permissions and contact/audit retention), production environment choices, independent security review, production-sized restore, and rollout/rollback evidence remain external gates.

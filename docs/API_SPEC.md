...
| `GET`  | `/admin/blog/posts/:id/translations/:locale/export`    | Authenticated `content.draft.read`; deterministic UTF-8 Markdown attachment from the exact saved translation. No autosave/locale fallback; digest mismatch is refused. Private, `no-store`, `noindex`; no content mutation |
...
| `POST` | `/admin/blog/posts/:id/translations/:locale/schedule`  | Future UTC timestamp; atomically persists scheduling intent, revision, audit, and invalidation in PostgreSQL                                                                                                               |
...
| `POST` | `/admin/blog/posts/:id/translations/:locale/archive`   | Explicit versioned archive command; blocked while `PUBLISHED`. Retains saved source and immutable history; records audit and invalidation                                                                                  |
...
| `POST` | `/admin/revisions/:id/restore`                         | Revalidates and re-renders recorded source through the transactional save path, creating a new revision; never mutates history or publication state                                                                        |

...
The active proof backlog is now M3–M6: the content store still needs a protected real GitHub branch/App run, M4 needs real reconciled bilingual content plus end-to-end publication invalidation, M5 needs its browser/font verification matrix, and M6 needs the API/auth/admin boundary. See [ROADMAP.md](ROADMAP.md) and [STATUS.md](STATUS.md) for the evidence-backed details.

...
- Every enabled theme MUST pass WCAG 2.2 AA contrast for body text, muted text, links, focus rings, borders that carry meaning, and both Shiki code themes. An enabled theme failing contrast is a release blocker, not a design preference. `checkThemeContrast` in `@portfolio/contracts/appearance` measures the thirteen pairings; text is held to SC 1.4.3 (4.5:1) and borders and the focus ring to SC 1.4.11 (3:1), which is the correct bound for a non-text boundary. **The code tokens are declared and measured but not yet wired:** no CSS selects between Shiki's light and dark output, so code blocks still render in one theme regardless of the selected one.

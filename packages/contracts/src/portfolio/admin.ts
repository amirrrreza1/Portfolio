...
export const adminSiteSettingsSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (new Set(value.enabledLocales).size !== value.enabledLocales.length) {
      ctx.addIssue({ code: "custom", path: ["enabledLocales"], message: "Locales must be unique." });
    }
    if (value.githubUsername === null && value.githubRepoAllowlist.length > 0) {
      ctx.addIssue({ code: "custom", path: ["githubRepoAllowlist"], message: "A GitHub username is required when repositories are enabled." });
    }
  });
...
export const adminSiteSettingsTranslationSchema = z.object({
  siteName: shortText,
  titleTemplate: trimmedTextSchema({ max: 240 }),
  metaDescription: prose.pipe(z.string().max(500)),
}).strict();

export const adminAppearanceUpdateSchema = z.object({
...
export const adminSectionUpdateSchema = z.discriminatedUnion("key", [
  z.object({ key: z.literal("hero"), content: heroBaseSchema, enabled: z.boolean(), sortOrder: sortOrderSchema }).strict(),
  z.object({ key: z.literal("about"), content: aboutBaseSchema, enabled: z.boolean(), sortOrder: sortOrderSchema }).strict(),
  ...(["skills", "projects", "certificates", "contact"] as const).map((key) => z.object({ key: z.literal(key), content: z.object({}).strict(), enabled: z.boolean(), sortOrder: sortOrderSchema }).strict()),
]);

export const adminSectionTranslationSchema = z.discriminatedUnion("key", [
  z.object({ key: z.literal("hero"), translation: heroTranslationSchema }).strict(),
  z.object({ key: z.literal("about"), translation: aboutTranslationSchema }).strict(),
  ...(["skills", "projects", "certificates", "contact"] as const).map((key) => z.object({ key: z.literal(key), translation: collectionTranslationSchema }).strict()),
]);

const localizedLabel = localizedMapSchema(shortText, { requireDefaultLocale: true }).strict();
export const adminNavItemCreateSchema = z.discriminatedUnion("targetKind", [
  z.object({ labelByLocale: localizedLabel, targetKind: z.literal("SECTION_ANCHOR"), target: stableKeySchema, iconKey: stableKeySchema.nullable(), enabled: z.boolean(), sortOrder: sortOrderSchema }).strict(),
  z.object({ labelByLocale: localizedLabel, targetKind: z.literal("INTERNAL_ROUTE"), target: internalPathSchema, iconKey: stableKeySchema.nullable(), enabled: z.boolean(), sortOrder: sortOrderSchema }).strict(),
]);
...
const socialBase = {
  ...
  rel: z.string().trim().min(1).max(120).regex(/^[a-z][a-z0-9-]*(?: [a-z][a-z0-9-]*)*$/).nullable(),
  enabled: z.boolean(),
  sortOrder: sortOrderSchema,
};
export const adminSocialLinkCreateSchema = z.discriminatedUnion("kind", [
  z.object({ ...socialBase, kind: z.literal("SOCIAL"), url: httpsUrlSchema }).strict(),
  z.object({ ...socialBase, kind: z.literal("DONATE"), url: httpsUrlSchema }).strict(),
  z.object({ ...socialBase, kind: z.literal("EMAIL"), url: mailtoUrlSchema }).strict(),
]);
...
export type AdminSiteSettingsTranslation = z.infer<typeof adminSiteSettingsTranslationSchema>;

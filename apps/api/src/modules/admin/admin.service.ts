...
export class AdminPortfolioService {
  ...
  async dashboard(): Promise<unknown> {
    ...
    const [
      ...
      this.database.auditEvent.findMany({
        where: { outcome: "FAILURE", createdAt: { gte: auditCutoff } },
        orderBy: { createdAt: "desc" },
        take: 10,
        select: {
          id: true,
      ...
      this.database.contentInvalidationOutbox.groupBy({
        by: ["state"],
        _count: { _all: true },
      }),
      this.database.contentJob.groupBy({
        by: ["state"],
        _count: { _all: true },
      }),
    ...
    return {
      ...
      delivery: {
        invalidations: Object.fromEntries(
          outbox.map((row) => [row.state, row._count._all])
        ),
  ...
  async updateSettings(
    ...
    return this.database.$transaction(async (tx) => {
      ...
      if (before === null)
        throw new Error("Site settings singleton is missing.");
      await assertMediaReference(
        tx,
        input.settings.defaultSocialImageId,
        "IMAGE"
      );
      ...
      const after = await tx.siteSettings.findUniqueOrThrow({
        where: { id: 1 },
      });
      await this.recordChange(
        ...
        after.version,
        "UPDATE",
        redactSettings(before),
        redactSettings(after)
      );
      return after;
    });
  }
  ...
  async readAppearance(): Promise<unknown> {
    return this.database.appearanceSettings.findUniqueOrThrow({
      where: { id: 1 },
    });
  }
  ...
  async updateAppearance(
    ...
    return this.database.$transaction(async (tx) => {
      const before = await tx.appearanceSettings.findUnique({
        where: { id: 1 },
      });
      if (before === null)
        throw new Error("Appearance settings singleton is missing.");
      await requireVersion(
        tx.appearanceSettings,
        "AppearanceSettings",
        "1",
        version,
        {
      ...
      const after = await tx.appearanceSettings.findUniqueOrThrow({
        where: { id: 1 },
      });
      await this.recordChange(
        tx,
  ...
  async listNavigation(): Promise<unknown> {
    return this.database.navItem.findMany({
  ...
  async createNavigation(
    actorId: string,
    input: AdminNavItemCreate
  ): Promise<unknown> {
    return this.database.$transaction(async (tx) => {
      const item = await tx.navItem.create({
        data: { ...input, labelByLocale: input.labelByLocale as never },
      });
      await this.recordChange(
        tx,
        actorId,
  ...
  async updateNavigation(
    ...
    return this.updateVersioned(
      ...
      id,
      version,
      { ...input, labelByLocale: input.labelByLocale }
    );
  }
  ...
  async listSkillCategories(): Promise<unknown> {
    return this.database.skillCategory.findMany({
      include: {
      ...
      orderBy: [{ archivedAt: "asc" }, { sortOrder: "asc" }, { key: "asc" }],
    });
  }

  async createSkillCategory(
    actorId: string,
    input: AdminSkillCategory
  ): Promise<unknown> {
    return this.database.$transaction(async (tx) => {
  ...
  async updateSkillCategoryTranslation(
    ...
    locale: "en" | "fa",
    version: number,
    input: AdminSkillCategoryTranslation
  ): Promise<unknown> {
    return this.database.$transaction(async (tx) => {
      const before = await tx.skillCategoryTranslation.findUnique({
        where: { categoryId_locale: { categoryId: id, locale } },
      });
      const translation = await upsertTranslation(
        tx.skillCategoryTranslation,
        "SkillCategoryTranslation",
        `${id}:${locale}`,
        version,
        { categoryId_locale: { categoryId: id, locale } },
        { categoryId: id, locale, ...input },
        input
      );
      await this.recordChange(
        tx,
  ...
  async listProjects(): Promise<unknown> {
    return this.database.project.findMany({
      ...
      orderBy: [{ archivedAt: "asc" }, { sortOrder: "asc" }, { slug: "asc" }],
    });
  }
  ...
  async updateProject(
    ...
    return this.database.$transaction(async (tx) => {
      ...
      if (before === null)
        throw new OptimisticConcurrencyError("Project", id, version, null);
      await requireVersion(
        tx.project,
        "Project",
        id,
      ...
      await tx.projectSkill.createMany({
        data: input.skills.map((skill) => ({ projectId: id, ...skill })),
      });
      const after = await tx.project.findUniqueOrThrow({
        where: { id },
        include: { skills: { orderBy: { sortOrder: "asc" } } },
  ...
  async updateProjectTranslation(
    ...
    locale: "en" | "fa",
    version: number,
    input: AdminProjectTranslation
  ): Promise<unknown> {
    return this.database.$transaction(async (tx) => {
      const before = await tx.projectTranslation.findUnique({
        where: { projectId_locale: { projectId: id, locale } },
      });
      const translation = await upsertTranslation(
        tx.projectTranslation,
        "ProjectTranslation",
        `${id}:${locale}`,
        version,
        { projectId_locale: { projectId: id, locale } },
        { projectId: id, locale, ...input },
        input
      );
      await this.recordChange(
        tx,
  ...
  async listCertificates(): Promise<unknown> {
    return this.database.certificate.findMany({
      include: { translations: { orderBy: { locale: "asc" } } },
      orderBy: [
        { archivedAt: "asc" },
        { sortOrder: "asc" },
  ...
  async updateCertificateTranslation(
    ...
    locale: "en" | "fa",
    version: number,
    input: AdminCertificateTranslation
  ): Promise<unknown> {
    return this.database.$transaction(async (tx) => {
      const before = await tx.certificateTranslation.findUnique({
        where: { certificateId_locale: { certificateId: id, locale } },
      });
      const translation = await upsertTranslation(
        tx.certificateTranslation,
        "CertificateTranslation",
        `${id}:${locale}`,
        version,
        { certificateId_locale: { certificateId: id, locale } },
        { certificateId: id, locale, ...input },
        input
      );
      await this.recordChange(
        tx,
  ...
  async listQuotes(): Promise<unknown> {
    return this.database.quote.findMany({
      orderBy: [
        { archivedAt: "asc" },
        { pinned: "desc" },
        { sortOrder: "asc" },
  ...
  async createQuote(actorId: string, input: AdminQuote): Promise<unknown> {
    return this.database.$transaction(async (tx) => {
      const quote = await tx.quote.create({
        data: { ...input, textByLocale: input.textByLocale as never },
      });
      await this.recordChange(
        tx,
        actorId,
  ...
  async updateQuote(
    ...
    return this.updateVersioned(
      ...
      id,
      version,
      { ...input, textByLocale: input.textByLocale }
    );
  }
  ...
  async uploadMedia(
    ...
    if (this.mediaStore === undefined) {
      throw new AdminInvariantError({
        media: ["Media storage is unavailable."],
      });
    }
  ...
  async updateMedia(
    ...
    return this.database.$transaction(async (tx) => {
      ...
      if (current === null)
        throw new AdminResourceNotFoundError("MediaAsset", id);
      if (current.kind === "IMAGE" && input.altText === null) {
        throw new AdminInvariantError({
          altText: ["Images require alternative text."],
        });
      }
      return this.updateVersionedInTransaction(
        tx,
    ...
    return this.database.$transaction(async (tx) => {
      ...
      if (references > 0) {
        throw new AdminResourceReferencedError({
          media: [
            `Detach this asset from its ${references} reference(s) first.`,
          ],
        });
      }
      return this.archiveInTransaction(
        tx,
        tx.mediaAsset,
        actorId,
        "MediaAsset",
  ...
  async listResumes(): Promise<unknown> {
    return this.database.resumeVersion.findMany({
      include: {
        mediaAsset: {
          select: {
            id: true,
  ...
  async activateResume(
    ...
    return this.database.$transaction(async (tx) => {
      const target = await tx.resumeVersion.findUnique({
        where: { id },
        include: { mediaAsset: true },
      });
      if (target === null)
        throw new AdminResourceNotFoundError("ResumeVersion", id);
      if (
        target.mediaAsset.archivedAt !== null ||
        target.mediaAsset.processingState !== "VERIFIED" ||
        target.mediaAsset.mimeType !== "application/pdf"
      ) {
        throw new AdminInvariantError({
          resume: ["The selected PDF is not available for activation."],
        });
      }
      ...
      const current = await tx.resumeVersion.findFirst({
        where: { activatedAt: { not: null }, retiredAt: null, id: { not: id } },
      });
      if (current !== null) {
        await requireVersion(
          tx.resumeVersion,
        ...
        const retired = await tx.resumeVersion.findUniqueOrThrow({
          where: { id: current.id },
        });
        await this.recordChange(
          tx,
      ...
      await requireVersion(tx.resumeVersion, "ResumeVersion", id, version, {
        activatedAt: now,
        retiredAt: null,
      });
      const active = await tx.resumeVersion.findUniqueOrThrow({
        where: { id },
      });
      await this.recordChange(
        tx,
  ...
  async archiveResource(
    ...
    return this.database.$transaction(async (tx) => {
      if (resource === "skill-categories") {
        const references = await tx.skill.count({
          where: { categoryId: id, archivedAt: null },
        });
        if (references > 0)
          throw new AdminResourceReferencedError({
            category: ["Archive or move its skills first."],
          });
      }
      if (resource === "skills") {
        const references = await tx.projectSkill.count({
          where: { skillId: id },
        });
        if (references > 0)
          throw new AdminResourceReferencedError({
            skill: ["Detach this skill from projects first."],
          });
      }
  ...
  async restoreRevision(actorId: string, revisionId: string): Promise<unknown> {
    ...
    return this.database.$transaction(async (tx) => {
      ...
      if (snapshot === null) {
        throw new AdminInvariantError({
          revision: ["This revision has no restorable snapshot."],
        });
      }
      ...
      switch (revision.entityType) {
        case "SiteSettings": {
          ...
          if (current === null)
            throw new AdminResourceNotFoundError("SiteSettings", "1");
          const parsed = parseRestore(adminSiteSettingsUpdateSchema, {
            settings: {
              ...
              birthDate:
                current.birthDate === null ? null : dateOnly(current.birthDate),
            },
          });
          await assertMediaReference(
            tx,
            parsed.settings.defaultSocialImageId,
            "IMAGE"
          );
          return this.updateVersionedInTransaction(
            tx,
        ...
        case "PageSection": {
          const parsed = parseRestore(
            adminSectionUpdateSchema,
            pick(snapshot, ["key", "content", "enabled", "sortOrder"])
          );
          return restoreCurrent(
            this,
            tx,
        ...
        case "SkillCategory": {
          const parsed = parseRestore(
            adminSkillCategorySchema,
            pick(snapshot, ["key", "enabled", "sortOrder"])
          );
          return restoreCurrent(
            this,
            tx,
        ...
        case "Project": {
          const skills = Array.isArray(snapshot.skills)
            ? snapshot.skills.map((item) =>
                pick(asRecord(item) ?? {}, ["skillId", "sortOrder"])
              )
            : [];
          ...
          if (current === null)
            throw new AdminResourceNotFoundError("Project", revision.entityId);
          await requireVersion(
            tx.project,
            "Project",
            revision.entityId,
          ...
          await tx.projectSkill.deleteMany({
            where: { projectId: revision.entityId },
          });
          await tx.projectSkill.createMany({
            data: parsed.skills.map((skill) => ({
              projectId: revision.entityId,
              ...skill,
            })),
          });
          const after = await tx.project.findUniqueOrThrow({
        ...
        case "MediaAsset": {
          const parsed = parseRestore(
            adminMediaUpdateSchema,
            pick(snapshot, ["displayName", "altText", "visibility"])
          );
          return restoreCurrent(
            this,
            tx,
        ...
        case "ResumeVersion": {
          const parsed = parseRestore(
            adminResumeUpdateSchema,
            pick(snapshot, ["label", "publicFilename"])
          );
          return restoreCurrent(
            this,
            tx,
        ...
        case "ProjectTranslation":
          return restoreTranslation(
            ...
            revision,
            adminProjectTranslationSchema,
            pick(snapshot, ["title", "summary", "longDescription"])
          );
        ...
        case "CertificateTranslation":
          return restoreTranslation(
            ...
            revision,
            adminCertificateTranslationSchema,
            pick(snapshot, ["title", "description"])
          );
        case "PageSectionTranslation": {
          ...
          if (section === null)
            throw new AdminResourceNotFoundError("PageSection", sectionId);
          ...
          return restoreTranslation(
            ...
            actorId,
            revision,
            { parse: () => parsed.translation },
            parsed.translation
          );
        }
        default:
          throw new AdminInvariantError({
            revision: ["This resource family cannot be restored here."],
          });
      }
    });
  }
  ...
  async createUser(actorId: string, input: AdminUserCreate): Promise<unknown> {
    if (this.recoverySecret === undefined) {
      throw new AdminInvariantError({
        user: ["User provisioning is unavailable."],
      });
    }
  ...
  async recordChange(
    ...
    await Promise.all([
      ...
      tx.auditEvent.create({
        data: {
          actorId,
          eventType: `admin.${entityType.toLowerCase()}.${action.toLowerCase()}`,
...
function invalidationRows(tx: any, entityType: string): Promise<unknown>[] {
  const namespaces =
    entityType === "AppearanceSettings"
      ? ["appearance"]
      : entityType.startsWith("Project") || entityType.startsWith("Skill")
        ? ["projects", "home"]
        : ["site", "home"];
  return (["en", "fa"] as const).map((locale) => {
    const tags = namespaces.map((namespace) =>
      publicCacheTag(namespace, locale)
    );
...
async function requireVersion(
  ...
  const result = await delegate.updateMany({
    where: { id: id === "1" ? 1 : id, version: expected },
...
async function upsertTranslation(
  ...
  if (current === null) {
    ...
    try {
      ...
      throw new AdminInvariantError({
        translation: ["The translation could not be created."],
      });
    }
  }
  const result = await delegate.updateMany({
...
async function assertProjectSkills(
  tx: any,
  ids: readonly string[]
): Promise<void> {
  ...
  if (count !== ids.length) {
    throw new AdminInvariantError({
      skills: ["Every selected skill must be active."],
    });
  }
}
...
function parseRestore<T>(
  ...
  if (!result.success || result.data === undefined) {
    throw new AdminInvariantError({
      revision: [
        "The historical snapshot no longer passes the current content contract.",
      ],
    });
  }
  return result.data;
...
function dateOnlyValue(value: unknown): string {
  ...
  if (result === null) {
    throw new AdminInvariantError({
      revision: ["The historical date is invalid."],
    });
  }
  return result;
}
...
async function restoreTranslation(
  ...
  if (parsed === undefined) {
    throw new AdminInvariantError({
      revision: ["The translation snapshot is invalid."],
    });
  }
  const snapshot = asRecord(revision.after);
  ...
  if (rowId.length === 0) {
    throw new AdminInvariantError({
      revision: ["The translation snapshot has no identity."],
    });
  }
